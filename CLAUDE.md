...
**Next unshipped work: OPS.2.5** — owner / tenant-admin flows (~9 `owner/*.spec.ts`, `owner-authed` project, §18.2 flows 1-head/2/3: property create, confirm/reject bookings). Then OPS.2.6 platform-admin + auth-edge (~6); OPS.2.7 nightly workflow + runbook + arch tests (incl. the 30-scenario count assertion — reconcile guest=10 + owner + platform-admin + anon 5 = 30 there); OPS.2.8 close-out + ADR-0019 + MASTER_PLAN row 17 flip. All authed scenarios go green only after the operator provisions the three Entra CIAM personas + KV passwords (OPS.2.8 §7 walk).
