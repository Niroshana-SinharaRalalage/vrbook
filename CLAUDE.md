...
**Slice OPS.2 — Playwright E2E suite (in progress 2026-07-09)**: architect plan committed as [`docs/OPS_2_PLAYWRIGHT_PLAN.md`](docs/OPS_2_PLAYWRIGHT_PLAN.md). All §5 policy questions locked to architect recommendation per owner directive 2026-07-09. 8 sub-commits planned; OPS.2.1 shipped (`21f5075`) — plan doc + `web/tests/e2e/` directory skeleton (6 subfolders + `.gitkeep` sentinels) + `README.md` + `.gitignore` entry for `web/tests/e2e/.auth/`.

**Next unshipped work: OPS.2.2** — personas + auth fixture + `global-setup.ts` running real MSAL sign-ins against staging Entra CIAM + Bicep `SeedE2eBackfill` service + `is_e2e` bool column on `identity.tenants` + three KV secrets `e2e-{guest,owner,platform-admin}-password` seeded as `pending-identity-setup` placeholders. Per plan §5-Q2-a: real MSAL redirect (Entra-local email+password personas, no MFA, no CA). Fallback to msal-node ROPC grant documented if MSAL headless flakes. Session budget: ~1 session. Then OPS.2.3-2.6 land the 30 scenarios; OPS.2.7 the nightly workflow + runbook + arch tests; OPS.2.8 the close-out + ADR-0019 + MASTER_PLAN row 17 flip.
