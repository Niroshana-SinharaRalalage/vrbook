...
**Next unshipped work: OPS.2.7** — `.github/workflows/nightly-playwright.yml` (cron `0 6 * * *`, fetches persona passwords from KV, runs the 3 authed projects, `continue-on-error` during the landing window) + `docs/runbooks/playwright-e2e-flake.md` + cross-surface arch tests (31-scenario count, `.auth/` gitignored, no new `[AllowAnonymous]`, no test-only middleware in prod `Program.cs`). Then OPS.2.8 close-out + ADR-0019 + MASTER_PLAN row 17 flip + the operator persona-provisioning walk that first turns the authed specs green.
