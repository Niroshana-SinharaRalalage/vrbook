...
**Next unshipped work: OPS.2.3** — anonymous smoke suite (5 `*.smoke.spec.ts`: home renders, property search, property detail by slug, unauthenticated quote calc, `/api/health` via browser context) + `playwright-smoke` job in `cd-staging-web.yml` after the curl smoke, **blocking from day one** (§5-Q4-a), Chromium only, against the deployed staging FQDN. Then OPS.2.4-2.6 land the authed scenarios (guest/owner/platform-admin, ~25); OPS.2.7 the nightly workflow + runbook + arch tests; OPS.2.8 the close-out + ADR-0019 + MASTER_PLAN row 17 flip.
