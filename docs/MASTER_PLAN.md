...
| | **Slice OPS.M.9 — RLS policies + `IRlsBypassDbContextFactory<TContext>` + bypass connection factory** | ⏭ **next** | same plan — **Phase-3 note**: decide per-statement vs per-connection `app.tenant_id` binding granularity in this slot, per [PHASE_3_RECONNAISSANCE](PHASE_3_RECONNAISSANCE.md). Per architect re-evaluation: M.9 owns the `IRlsBypassDbContextFactory<TContext>` contract authoring (do not author it earlier). | |
