...
| | Slice OPS.M.10 — Cross-tenant isolation test pack | ⚠ Wave 1 | `1ec66de` → `a1164b4` | EndpointCoverageArchTest + RLS schema fact pack (89 facts) + runbook shipped; full ~200-fact matrix deferred to Slice OPS.M.10 Wave 2 |
| **Phase 1.5 follow-up** | **Slice OPS.M.10 Wave 2 — full cross-tenant API matrix** | ⏭ **next (optional)** | follow-up to OPS.M.10 plan §11 deviations | `TwoTenantApiFixture` + `RouteMatrix` + ~200-fact `CrossTenantEndpointMatrix` |
