...
| | **Slice OPS.M.7 — Tenant Admin onboarding wizard** | ⏭ **next** | same plan — welcome email is operator-manual placeholder until Slice 4 ships; the `tenant.welcome` template + `TenantNotificationHandlers` land in Slice OPS.M.7's scope once the ACS pipeline exists (Slice 4) | |
| | Slice OPS.M.8 — Super Admin console | ⏭ | same plan | |
| | Slice OPS.M.9 — RLS policies + `IRlsBypassDbContextFactory<TContext>` + bypass connection factory | ⏭ | same plan — **Phase-3 note**: decide per-statement vs per-connection `app.tenant_id` binding granularity in this slot, per [PHASE_3_RECONNAISSANCE](PHASE_3_RECONNAISSANCE.md). Per architect re-evaluation: M.9 owns the `IRlsBypassDbContextFactory<TContext>` contract authoring (do not author it earlier). | |
