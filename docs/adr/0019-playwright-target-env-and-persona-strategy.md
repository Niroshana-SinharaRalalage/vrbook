...
2. **Real MSAL sign-in, never a fake-auth backdoor.** Authed personas sign in through the actual Entra CIAM hosted flow driven by the browser (`global-setup.ts`, setup project). ADR-0016 (admin surface Entra-local only) + ADR-0017 (admins pre-seeded) forbid an `[AllowAnonymous]`/DevAuth shortcut on the admin surface — enforced by arch test `OpsOps2_AdminSurfaceAndTestBackdoorTests`. **There is NO ROPC fallback** (amended 2026-07-20, architect consult): ROPC (`grant_type=password`) bypasses the CIAM user flow, so its tokens carry neither the `tfp`/`acr` admin-flow marker nor the flow's application-claims mapping (its `email` resolves to the synthetic `<guid>@…onmicrosoft.com` UPN, not the seeded `@vrbook.test`), and it cannot satisfy the OPS.M.22 admin-preseed gate (ADR-0017). Making it "work" would require forging `tfp` via a claims policy — fabricating the exact admin-flow signal the gate verifies. The only supported admin-token path is the interactive `AdminSignUpSignIn` capture; if the hosted page drifts, fix the selectors/interstitial handling in `global-setup.ts` — never ROPC.
