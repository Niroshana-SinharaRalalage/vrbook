...
|---|---|---|---|---|
| G1 | ✅ **RESOLVED (VRB-206)** | ~~Loyalty tier thresholds hard-coded in domain; `Loyalty:*Threshold` config is dead~~ Fixed: `LoyaltyOptions` (Bind + `ValidateOnStart`, defaults 1/3/6) feeds `LoyaltyThresholds` into the pure domain (`TierDefinition.Resolve`/`NextTier`, `RecordCompletedStay`); the `Loyalty:*Threshold` config + Bicep vars now bind and take effect. Discounts stay fixed (Q2). | `LoyaltyOptions.cs`; `LoyaltyAccount.cs`; `OnBookingCompletedHandler.cs`/`GetMyLoyaltyQuery.cs` | Closed — ops can tune tiers via config, no code change |
| G2 | ✅ **RESOLVED (VRB-207)** | ~~Booking Tentative SLA hard-coded to 24h; `Booking:TentativeSlaHours` never read; Bicep comment says "6h"~~ Fixed: `BookingSlaOptions` (Bind + `ValidateOnStart`, owner-locked default **48h**) fed into the pure domain (`Booking.Place(..., tentativeSla)`) via `PlaceBookingHandler`; the expiry sweep honours the stamped `TentativeUntil` (one source of truth); stale Bicep "6h" comment corrected. | `BookingSlaOptions.cs`, `Booking.cs`, `PlaceBookingHandler.cs`; `main.bicep` | Closed — one tunable, correct 48h value (24h→48h is behaviour-changing, Q1-locked) |
| G3 | ✅ **RESOLVED (VRB-208)** | ~~`Booking:HoldDurationMinutes` + `Sync:DefaultPollIntervalMinutes`/`StaleAlertHours` have no consumers~~ Fixed: hold key **wired** → `BookingHoldOptions` (Bind + `ValidateOnStart`) → `CreateHoldHandler` TTL (was hard-coded 15-min const); the two dead Sync keys **removed** from appsettings+Bicep+matrix (cadence is per-feed DB). `DeadConfigTests` guards against re-creep. | `BookingHoldOptions.cs`, `HoldCommands.cs`; `DeadConfigTests.cs` | Closed — config surface tells the truth |
| G4 | ✅ **RESOLVED (VRB-208)** | ~~`Sync__DefaultPollIntervalMin` (Bicep) ≠ `Sync:DefaultPollIntervalMinutes` (appsettings)~~ Fixed: the misnamed Bicep var was deleted together with the dead appsettings key | `main.bicep` | Closed — no orphan/mismatch |
...
| G7 | ✅ **RESOLVED (VRB-209)** | ~~`EntraExternalId:AdminFlowName` read but never provided; admin-flow gate inert until set~~ Fixed: `AdminFlowName` now provided in appsettings + Bicep-plain, read via bound `IOptions<EntraExternalIdOptions>` (was a raw `configuration[...]`), and **fail-fast required in Production** (Staging exempt with an inert placeholder until OPS.M.22 ships the real Entra admin flow). `Payment:AllowPlatformFallback` also made explicit per env (was referenced, never set). | `EntraExternalIdOptions`, `ConfigValidators.cs`, `UserProvisioningMiddleware.cs`; `main.bicep`, `appsettings.json` | Closed — admin-gate config is provided + prod-enforced; no staging-outage risk |
| G8 | ✅ **RESOLVED (VRB-205 item 3)** | ~~Hard-coded staging API FQDN in web build-arg~~ Fixed: `cd-staging-web.yml` now resolves `NEXT_PUBLIC_API_BASE_URL` from the **live Container App FQDN** (`az containerapp show`) at build time (bare origin — the web client appends `/api/v1`); `main.bicep` also exports an `apiBaseUrl` output | `cd-staging-web.yml` (Resolve API base URL step); `main.bicep` `apiBaseUrl` output | Closed — survives any CAE environment rebuild |
