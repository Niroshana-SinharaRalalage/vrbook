...
Note the split of concerns: the **matrix + arch test** guarantee *every endpoint is authorization-covered and accounted for*; the **per-module `Contract/*` classes** (`tests/VrBook.Api.IntegrationTests/Contract/<Module>/*`, `Category=Integration`) add the *happy-path body / validation→400 / error-contract (`problem+json` `type`) / idempotency* assertions **and the per-resource cross-tenant isolation** (OwnerB reaching TenantA's seeded property → RLS 404) that the status-set matrix intentionally does not make. Both are required for an endpoint to be "covered."
