...
### VRB-300 — Complete the API contract suite + close the endpoint-coverage gate
- **Epic:** Go-Live · **Priority:** Must · **Estimate:** L · **Lane:** TEST · **Wave 0 — lands first, before any feature lane claims.**
- **Narrative:** As any agent shipping into a codebase many agents share, I want **every** API endpoint exercised through the real middleware pipeline against a real Postgres across all contract dimensions, plus a build-time gate that fails when an endpoint is not covered, so "the suite is green" is a real merge precondition. Strategy + tooling: [`../TEST-STRATEGY.md`](../TEST-STRATEGY.md).
- **⚠️ Build on what exists — do NOT rebuild (this story was rescoped after the audit found existing machinery).** Slice OPS.M.10 already shipped: `tests/VrBook.Api.IntegrationTests/Multitenancy/RouteMatrix.cs` (the single-source enumeration of endpoint×persona×tenant, **~29 of ~98 actions today**, each a `Cell` with an `AcceptedStatuses` set + optional `BodyFactory`); `CrossTenantEndpointMatrix.cs` (the `[Theory]` that drives it); `EndpointCoverageArchTest.cs` (asserts every action carries an explicit access decision — but **its own XML-doc admits the "appears in RouteMatrix OR is `[ExemptFromCrossTenantMatrix]`" second half is not yet enforced**); and `ExemptFromCrossTenantMatrixAttribute` in `VrBook.Api.Common`. Plus ~59 integration test classes. VRB-300 **completes and extends** this substrate; it does not create a parallel one.
- **Acceptance criteria:**
  - **Close the coverage gate (the real gate):** **Given** `EndpointCoverageArchTest`, **when** VRB-300 lands, **then** it is strengthened to enforce the **second half of the documented M.10 invariant** — every authenticated controller action **either appears in `RouteMatrix.GetAll()` or carries `[ExemptFromCrossTenantMatrix("reason")]`** — and the build **fails naming any action** that is neither. (Today it only checks an access-decision attribute exists.)
  - **Extend the matrix to full coverage:** **Given** the ~98 controller actions vs the ~29 rows today, **then** `RouteMatrix` is extended so every non-exempt authenticated action has at least its **auth + cross-tenant-isolation** row(s) — so the strengthened gate passes without blanket exemptions. Exemptions are used only for genuinely un-matrixable actions, each with a documented reason.
  - **Add the contract dimensions the isolation matrix does not test:** **Given** each endpoint, **then** per-module contract test classes on `TwoTenantApiFixture` add **happy-path body assertions · input validation (→400 problem shape) · error contract (documented status + `problem+json` `type`) · idempotency** where the endpoint promises it. (The `AcceptedStatuses` matrix proves *authorization*, not these — they need explicit assertions.)
  - **Given** anonymous (`CreateClientAs(null)`), **then** authed routes return 401; **given** `OwnerB` at a `TenantA` resource, **then** RLS/`HasTenantRole` denies it (not merely a controller check).
  - **Given** CI, **then** the suite is **blocking** on `develop` and wired into `cd-prod.yml` (VRB-301) before the approval gate; **given** Docker up, `dotnet test tests/VrBook.Api.IntegrationTests --filter Category=Integration` matches CI green/red.
- **TDD plan:** (1) **First strengthen the arch test** to require matrix-membership-or-exempt, and watch it **fail listing every currently-uncovered action** (~69 of them) — this is the honest RED. (2) Drive `RouteMatrix` rows + per-module contract classes until it goes green. (3) Prove the gate bites: delete a matrix row for a non-exempt action → arch test red. Seed extra fixtures inside the test under `RlsBypassScope` via the real domain factories. Everything `Category=Integration`.
- **Technical notes:** extend the **existing** files — `RouteMatrix.cs`, `EndpointCoverageArchTest.cs` — and add `tests/VrBook.Api.IntegrationTests/Contract/<Module>/*` for the non-auth dimensions. Assert error bodies as **status + problem `type`** only (Hellang strips custom fields — [`reference_problem_details_strips_body`]). No test-only production bypass (`OpsOps2_AdminSurfaceAndTestBackdoorTests` guards it). This substrate is where every feature lane's per-endpoint tests land (ENGINEERING-RULES §3).
- **Configuration:** none new. CI already runs `VrBook.Api.IntegrationTests` + `VrBook.Architecture.Tests`; confirm both are blocking and included in `cd-prod.yml` pre-approval.
- **Rollout:** lands to `develop` as the first Wave-0 story; immediately blocking. No runtime/app change ships.
- **Observability:** CI run summary publishes covered-action count (matrix rows + exemptions vs total actions); a drop is a reviewer red flag.
- **Definition of Done:** strengthened `EndpointCoverageArchTest` enforces matrix-or-exempt and is proven to fail when a row is removed → `RouteMatrix` + per-module contract classes cover every action across the dimensions → suite blocking on `develop` + in `cd-prod.yml` → [`../TEST-STRATEGY.md`](../TEST-STRATEGY.md) reflects the final shape → board row `DONE`.
- **Dependencies:** blocked-by nothing (foundation). **Every endpoint-touching story consumes it** — new endpoints add a `RouteMatrix` row + contract tests as part of that story (ENGINEERING-RULES §3), or the strengthened gate fails their PR.
- **Parallelisation:** Lane = TEST (Wave 0). Owns `tests/VrBook.Api.IntegrationTests/Multitenancy/RouteMatrix.cs` + `Contract/*` + `tests/VrBook.Architecture.Tests/EndpointCoverageArchTest.cs`. Runs parallel to CONFIG/DESIGN/DEVOPS-prereq; touches no feature code, so it never collides.
...
> **Delivery status (2026-07-16, DONE):** the **design/docs** legs shipped —
...
> runtime failure = exit 1 + `Fatal` — HostAbortedException is design-time-only).
> **Zero-downtime migration observed:** `cd-staging-api` run **29466376892** (SHA 722a9ba) —
> `migrate (Postgres)` → `deploy api` → `smoke (api)` all green, in order (migrator completed
> before the revision promote = old revision served throughout; post-deploy smoke passed).
> **Deferred, not dropped** (documented in the drill doc): (a) live forward-fix + twice-run
> idempotency drills (need a **write**/`job start` — idempotency is verified-by-design in code);
> (b) the **cd-prod.yml pre-deploy migration gate** (rides the deferred prod pipeline, VRB-301/302);
> (c) the **PITR recovery drill** (blocked-by VRB-304 — forward-fix path documented; PITR mechanics defer to VRB-304).
- **Epic:** Go-Live · **Priority:** Must · **Estimate:** M
