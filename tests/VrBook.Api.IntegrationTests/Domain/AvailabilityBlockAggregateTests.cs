...
/// <summary>
/// Slice 3 — unit tests for the AvailabilityBlock aggregate invariants.
...
public sealed class AvailabilityBlockAggregateTests
{
    ...
    {
        var block = AvailabilityBlock.Create(AnyPropertyId, Day1, Day3, reason: null);
        ...
        block.TenantId.Should().BeNull(
            "Phase 1 leaves tenant_id null per REPLAN §10.1; OPS.M.3b backfills.");
    }

    [Fact]
    public void Create_trims_reason_and_treats_whitespace_as_null()
    {
        AvailabilityBlock.Create(AnyPropertyId, Day1, Day3, "  maintenance ").Reason.Should().Be("maintenance");
        AvailabilityBlock.Create(AnyPropertyId, Day1, Day3, "   ").Reason.Should().BeNull();
    }

    [Fact]
    public void Create_rejects_empty_property_id()
    {
        var act = () => AvailabilityBlock.Create(Guid.Empty, Day1, Day3, null);
    ...
    {
        var act = () => AvailabilityBlock.Create(AnyPropertyId, Day1, Day1, null);
    ...
    {
        var act = () => AvailabilityBlock.Create(AnyPropertyId, Day3, Day1, null);
        act.Should().Throw<ArgumentException>().WithParameterName("endDate");
    }
    ...
    {
        var act = () => AvailabilityBlock.Create(AnyPropertyId, Day1, Day3, new string('x', 201));
    ...
    {
        var b = AvailabilityBlock.Create(AnyPropertyId, Day1, Day3, null);
